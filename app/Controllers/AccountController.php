...
class AuthController extends Controller
{
    ...
    // Tetapkan grup sebelum menyimpan pengguna
    $group = $this->request->getVar('group');
    if (! empty($group)) {
        $users->withGroup($group);
    } else {
        // Jika tidak ada grup yang dipilih, atur ke grup default
        $users->withGroup($this->config->defaultUserGroup);
    }

    $this->config->requireActivation === null ? $user->activate() : $user->generateActivateHash();

    // Simpan pengguna setelah menetapkan grup
    if (! $users->save($user)) {
